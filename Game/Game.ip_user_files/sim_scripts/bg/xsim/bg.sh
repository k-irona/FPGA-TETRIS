...
#
# Script generated by Vivado on Mon Dec 15 20:27:29 +0800 2025
...
#
# Usage        : bg.sh
#                bg.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                bg.sh [-reset_run]
#                bg.sh [-reset_log]
#                bg.sh [-help]
#
...
# script info
echo -e "bg.sh - Script generated by export_simulation (Vivado v2023.1 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./bg.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  xelab --incr --debug typical --relax --mt 2  -L blk_mem_gen_v8_4_6 -L xil_defaultlib -L unisims_ver -L unimacro_ver -L secureip -L xpm --snapshot bg xil_defaultlib.bg xil_defaultlib.glbl -log elaborate.log
}

# RUN_STEP: <simulate>
simulate()
{
  xsim bg -key {Behavioral:sim_1:Functional:bg} -tclbatch cmd.tcl -log simulate.log
}
...
reset_run()
{
  files_to_remove=(xelab.pb xsim.jou xvhdl.log xvlog.log compile.log elaborate.log simulate.log xelab.log xsim.log run.log xvhdl.pb xvlog.pb bg.wdb xsim.dir libdpi.so)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./bg.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: bg.sh [-help]\n\
Usage: bg.sh [-step]\n\
Usage: bg.sh [-lib_map_path]\n\
Usage: bg.sh [-reset_run]\n\
Usage: bg.sh [-reset_log]\n\
Usage: bg.sh [-keep_index]\n\
Usage: bg.sh [-noclean_files]\n\n\
